...
class RedisAdapter(BaseAdapter):
    ...
    def __init__(self, *args):
        ...
        :param args: None
        """
        super(RedisAdapter, self).__init__()
        self.driver = None
        pass
    ...
    def _create_session(self):
        ...
        redis_graph = Graph('social', self.driver)
        def run (query, params=None):

            redis_graph.query(query,params=params)
        redis_graph.run = run
        return redis_graph
    ...
    def connect(self, authentication_options):
        ...
        # TODO: Use username and password args
        ...
        try:
            ...
            self.driver = r
        # TODO: Proper connection exception handling
    ...
    def _prefix_metadata(metadata):
        ...
        for key, value in metadata.items():
            prefixed_metadata["{meta_prefix}{key}".format(key=key, meta_prefix=REDIS_META_PREFIX)] = value
    ...
    def save_element(self, attributes, metadata):
        ...
        merge_relevant_keys.append(REDIS_META_PREFIX + "{}".format(METADATA_KEY_IDENTIFIER))
        ...
        attr_for_simple_set.remove(REDIS_META_PREFIX + METADATA_KEY_NAMESPACES)
        attr_for_simple_set.remove(REDIS_META_PREFIX + METADATA_KEY_TYPE_MAP)
        cypher_set_statement = self._get_attributes_set_cypher_string(attr_for_simple_set)

        attr_for_list_merge = list()
        attr_for_list_merge.append(REDIS_META_PREFIX + METADATA_KEY_NAMESPACES)
        attr_for_list_merge.append(REDIS_META_PREFIX + METADATA_KEY_TYPE_MAP)
        ...
        session.run(command, params=dict(db_attributes))

        # TODO: Support merge logic
    ...
    def save_relation(self, from_node, to_node, attributes, metadata):
        ...
        with self._create_session() as session:
            ...
            session.run(command, dict(db_attributes))
            # TODO: Proper support for Merge checks
            # with session.begin_transaction() as tx:
            #
            #     result = tx.run(command, dict(db_attributes))
            #
    ...
    def _split_attributes_metadata_from_node(db_node):
        ...
        # split data
        metadata = {k.replace(REDIS_META_PREFIX, ""): v for k, v in db_node._properties.items() if
                    k.startswith(REDIS_META_PREFIX, 0, len(REDIS_META_PREFIX))}
        attributes = {k: v for k, v in db_node._properties.items() if
                      not k.startswith(REDIS_META_PREFIX, 0, len(REDIS_META_PREFIX))}
    ...
    def get_relation(self, relation_id):
        ...
        relation = None
        for result in result_set:
            # TODO: Test if this Edge case is working
            if not isinstance(result["relation"], Edge):
                raise DatabaseException(
